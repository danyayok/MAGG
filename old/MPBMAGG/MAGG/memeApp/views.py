...
features = ["классические", "черный юмор", "политика", "постирония", "жиза", "шаблонные", "современные", "старые", "мудро"]
X = df[features]
y = df["popularity"]
...
def gpt_tesis(text):
    ...
    for i in range(10):  # Пробуем максимум 10 раз
        ...
        if res and 'Model' not in res and 'error' not in res and 'chat' not in res and 'date: {' not in res and len(res.split("|")) > 1:
            print("Ответ GPT получен!")
...
def get_results(request):
    ...
    tag_stats = df[tags].multiply(df["popularity"], axis="index").mean().sort_values(ascending=False).head(10) # 10 самых популярных тегов (я зря это сделал, у нас всего 10 тегов)
